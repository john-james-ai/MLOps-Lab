...
# Version    : 0.1.0                                                                               #
# Python     : 3.10.6                                                                              #
# Filename   : /mlops_lab/core/workflow/operator/train_test_split.py                               #
# ------------------------------------------------------------------------------------------------ #
# Author     : John James                                                                          #
...
class TrainTestSplit(Operator):
    ...
    def _build_dataset(self, train: pd.DataFrame, test: pd.DataFrame) -> Dataset:
        ...
        train_dataframe = dataset.create_dataframe(
            data=train, name=self._train_params.name, description=self._train_params.description
        )
        dataset.add_dataframe(train_dataframe)
        test_dataframe = dataset.create_dataframe(
            data=test, name=self._test_params.name, description=self._test_params.description
        )
