...
# Modified   : Sunday January 1st 2023 02:07:09 am                                                 #
# ------------------------------------------------------------------------------------------------ #
# License    : MIT License                                                                         #
...
class CreateJobTable(SQL):
    name: str = "job"
    sql: str = """CREATE TABLE IF NOT EXISTS job (id INTEGER PRIMARY KEY, oid TEXT GENERATED ALWAYS AS ('job_' || name || "_" || id || "_" || mode), name TEXT NOT NULL, description TEXT, mode TEXT NOT NULL, state TEXT DEFAULT "CREATED", created timestamp, modified timestamp);CREATE UNIQUE INDEX IF NOT EXISTS name_mode ON job(name, mode);"""
    args: tuple = ()
