...
class InsertTask(SQL):
    dto: DTO
    sql: str = """INSERT INTO task (name, description, mode, state, job_id, created, modified) VALUES (%s, %s, %s, %s, %s, %s, %s);"""
    args: tuple = ()
...
class UpdateTask(SQL):
    dto: DTO
    sql: str = """UPDATE task SET name = %s, description = %s, mode = %s, state = %s, job_id = %s, created = %s, modified = %s WHERE id = %s;"""
    args: tuple = ()
