...
from recsys.core.workflow.operators import (
    KaggleDownloader,
    DeZipper,
    Pickler,
    Copier,
...
class TestOperators:  # pragma: no cover
    ...
    # ============================================================================================ #
    def test_url_downloader(self, caplog):
        start = datetime.now()
        logger.info(
            "\n\tStarted {} {} at {} on {}".format(
                self.__class__.__name__,
        ...
        destination = "data/ext/movielens/ml-25m.zip"
        dl = Downloader(url=url, destination=destination)
        dl.run()
