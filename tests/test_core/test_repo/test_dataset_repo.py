...
# Modified   : Sunday January 1st 2023 07:25:41 am                                                 #
# ------------------------------------------------------------------------------------------------ #
# License    : MIT License                                                                         #
...
class TestDatasetRepo:  # pragma: no cover
    ...
    def test_setup(self, container, caplog):
        ...
        # ---------------------------------------------------------------------------------------- #
        table = container.table.dataset()
        table.reset()
        table = container.table.dataframe()
        table.reset()
        # ---------------------------------------------------------------------------------------- #
        end = datetime.now()
    ...
    # ============================================================================================ #
    def test_add_get(self, container, datasets, caplog):
        start = datetime.now()
        logger.info(
            "\n\n\tStarted {} {} at {} on {}".format(
                self.__class__.__name__,
        ...
        # ---------------------------------------------------------------------------------------- #
        repo = container.repos.dataset()
        for i, dataset in enumerate(datasets, start=1):
            dataset = repo.add(dataset)
            assert dataset.id == i
            j2 = repo.get(i)
            assert dataset == j2
            assert len(dataset) == len(j2)
        # ---------------------------------------------------------------------------------------- #
        end = datetime.now()
        duration = round((end - start).total_seconds(), 1)
    ...
    def test_get_by_name(self, container, caplog):
        ...
        for name, dataframe in dataset.dataframes.items():
            ...
            assert dataframe.dataset == dataset
        # ---------------------------------------------------------------------------------------- #
        end = datetime.now()
        duration = round((end - start).total_seconds(), 1)
    ...
    def test_get_all(self, container, caplog):
        ...
        for i, dataset in datasets.items():
            assert isinstance(dataset, Dataset)
            assert dataset.id == i
            assert dataset.oid is not None
            for j, dataframe in dataset.dataframes.items():
                assert isinstance(dataframe, DataFrame)
                assert dataframe.name == j
        # ---------------------------------------------------------------------------------------- #
        end = datetime.now()
        duration = round((end - start).total_seconds(), 1)
    ...
    def test_update(self, container, caplog):
        ...
        datasets = repo.get_all()
        for i, dataset in datasets.items():
            assert dataset.state == "COMPLETE"
            for j, dataframe in dataset.dataframes.items():
                assert dataframe.state == "COMPLETE"
